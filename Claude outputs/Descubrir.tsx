...
// aparece todo el contenido del scroll" / "pon seis o siete máximo y el
// resto en la pantalla de ver más"): bug real — cada sección de
// Escena/Cultura (SECCIONES_ESCENA/SECCIONES_CULTURA, ver
// data/experiences.ts) mostraba en el riel de Home la lista COMPLETA sin
// recortar (`items.map`), y `getVerMasContent` resuelve esa misma
// sección con el mismo filtro sin excluir nada — dos pantallas mostrando
// exactamente lo mismo, sin ninguna diferencia. Se recorta el riel de
// Home a este máximo (mismo criterio de "Home muestra una selección,
// Ver más muestra todo" que ya usan Más reservados/Descubrimientos/
// Curado); Ver más sigue sin tocarse, sigue resolviendo la lista
// completa real de la sección tal cual ya lo hacía.
const MAX_ITEMS_RIEL_CATEGORIA = 6;
          ...
          {(activeCategory === "Escena" || activeCategory === "Cultura") &&
            (activeCategory === "Escena" ? SECCIONES_ESCENA : SECCIONES_CULTURA).map(
              (seccion) => {
                ...
                // Ver nota grande arriba de MAX_ITEMS_RIEL_CATEGORIA: el
                // scroll de Home muestra como máximo esta cantidad, el
                // resto (si hay) solo se ve en `/ver-mas/${seccion.slug}`.
                const itemsVisibles = items.slice(0, MAX_ITEMS_RIEL_CATEGORIA);
                return (
