# Copy Confirmado

## Navbar
Ana Maria Serrano | Proyectos · Sobre mí · Contacto

## Hero

### Título
Hola soy Ana, y nunca doy por terminada una interfaz hasta que incluso el border radius también tenga sentido.

Porque cada detalle influye en la forma en que las personas entienden, perciben y confían en un producto.

### Nota de investigación (IBM Plex Mono, cursor escribiendo)
// Research notes:
> buscando sentido antes de diseñar soluciones.

## Cards de proyecto

### Beacon
**Label:** UX/UI · Product Design · AI
**Headline:** Diseñé y construí con IA una herramienta para recuperar miles de dólares en ventas indirectas.
**Descripción:** Pipeline, pulse checks automáticos y cero atribuciones perdidas.
**CTA:** Ver caso de estudio →

### Theaveling
**Label:** UX/UI · Research · Mobile
**Headline:** Cuando el arte necesita una hoja de ruta.
**Descripción:** Una app para viajeros y locales que buscan experiencias artísticas alternativas. Proceso completo de Design Thinking: research, benchmarking, testing con usuarios y UI Kit propio.
**CTA:** Ver caso de estudio →

## About Me

### Subtítulo
Del arte al diseño, sin soltar ninguno de los dos.

### Párrafos
Me formé en artes y eso me dio un ojo crítico que no se aprende en un bootcamp: cuido la tipografía, los bordes, los degradados. Cada detalle visual comunica algo, y sé exactamente qué.

Trabajé en Customer Experience antes de entrar al diseño de producto, y eso me cambió la perspectiva: diseño desde la experiencia real del usuario, no desde lo que se ve bonito en una presentación.

Hoy diseño con IA, entiendo el código y construyo soluciones que se ven bien y tienen impacto en el negocio. No debato de píxeles — los resuelvo.

## Contacto

### Título
¿Hablamos?

### Descripción
Estoy disponible para proyectos freelance y oportunidades de trabajo.

### Links
anserrarg@gmail.com
linkedin.com/in/anaserrano15

---

## Cards de proyectos (sección scroll)

### Beacon
**Etiqueta:** Investigación UX / Estrategia de Producto / Desarrollo con IA
**Descripción:**
Beacon es una herramienta web que diseñé desde cero con IA para ayudar a equipos comerciales B2B a no perder de vista oportunidades de hasta USD 400.000 anuales. Ayuda a detectar oportunidades en riesgo antes de que afecten las ventas y las comisiones.

### Theaveling
**Etiqueta:** Investigación UX / Design Thinking / Diseño de Interfaz
**Descripción:**
Aplicación móvil para ayudar a turistas a descubrir experiencias artísticas y culturales relevantes en nuevas ciudades. Proyecto desarrollado siguiendo un proceso completo de Design Thinking, desde la investigación hasta la validación del prototipo.
